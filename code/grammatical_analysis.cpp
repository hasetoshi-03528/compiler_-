...
void LR1::parser(vector<TOKEN> token_stream, const string file_path)
{
	...
	if (!file_out.is_open()) {
		//无法打开语法分析的输出文件
		cout << "Unable to open the output file of syntax analysis";
	...
	//输出产生式
	//步骤 状态 符号 输入串
	file_out << setw(print_format.step_len) << "Steps" << setw(print_format.status_len) << "Status" << setw(print_format.symbol_len) << "Symbols" << setw(print_format.input_len) << "Input string" << setw(print_format.production_len) << "Generative" << endl;
	...
	for (int i = 0; i < token_stream.size(); i++) {
		...
		if (current_token_index == -1) {
			//待分析字符流中出现了未在文法中进行定义的终结符
			cout << "A terminator is present in the stream of characters to be analyzed that is not defined in the grammar" << endl;
		...
		if (current_action_iter == action_table.end()) {
			//语法分析过程中在（第 行，第 列）发现错误
			cout << "Error found in (row " << token_stream[i].row << ", column " << token_stream[i].col << "during syntax analysis" << endl;
