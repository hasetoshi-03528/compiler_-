...
void grammar::read_grammar(const string file_path)
{
	...
	if (!file_in.is_open()) {
		//无法打开文法文件
		cout << "Unable to open grammar file" << endl;
	...
	while (getline(file_in, line, '\n')) {
		...
		if (production_left_and_right.size() != 2) {
			//第行的产生式格式有误（每行应有且只有一个符号）
			cout << "Wrong format of generating formula in line " << grammar_row_num << " (each line should have one and only one\"" << ProToken << "\" (each line should have one and only one" << endl;
			throw(GRAMMAR_ERROR);
		}
		...
		if (production_right_parts.size() == 0) {
			//第 行的产生式格式有误（产生式右端没有文法符号）
			cout << "The format of the generator in line " << grammar_row_num << "is wrong (no grammar symbol at the right end of the generator)" << endl;
			throw(GRAMMAR_ERROR);
		}
